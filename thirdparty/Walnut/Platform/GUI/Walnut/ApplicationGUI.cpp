...
#include "Walnut/Embed/WindowImages.embed"
	...
	/*	ImFontConfig fontConfig;
		...
		ImFontConfig fontConfig;
		fontConfig.FontDataOwnedByAtlas = false;
		ImFont* monacoFont = io.Fonts->AddFontFromFileTTF("Monaco.ttf", 16.f, &fontConfig);
		s_Fonts["Default"] = monacoFont;
		io.FontDefault = monacoFont;
		/*ImFontConfig fontConfig;
	...
	void Application::UI_DrawTitlebar(float& outTitlebarHeight)
	{
		...
		const float titlebarHeight = isMaximized ? 44.0f : 42.0f;
		...
		// Logo
		{
			...
			const ImVec2 logoOffset(8.0f + windowPadding.x, 5.0f + windowPadding.y + titlebarVerticalOffset);
		...
		if (m_MenubarCallback)
		{
			ImGui::SuspendLayout();
			{
				...
				const float logoHorizontalOffset = 8.0f * 2.0f + 32.0f + windowPadding.x;
...

		{
			...
			ImGui::SetCursorPos(ImVec2(ImGui::GetWindowWidth() * 0.5f - textSize.x * 0.5f, 2.0f + windowPadding.y + 8.0f));
